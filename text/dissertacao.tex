...
O uso de Análise Automatizada de Conteúdo tem se popularizado nas Ciências Sociais Aplicadas como um conjunto de técnicas que permitem a investigação em grandes bases de dados de \emph{corpus textuais} \cite{gentzkow2019text}. Na Ciência Política, a escrita registra boa parte da atividade de agentes políticos. Leis, documentos oficiais, discursos no plenário e manifestos registram boa parte das mobilizações instituicionais e civis nas dinâmicas de poder da sociedade e, portanto, há grande potencialidade para extração de conhecimento de fontes pouco exploradas \cite{wilkerson2017large}.

No que diz respeito a aplicabilidade dos modelos utilizados, algumas meta-análises dedicaram-se abordar o estado da arte da literatura e apontar seus principais avanços e dificuldades. \emph{Wordfish}, \emph{Wordscores} e \emph{Latent Dirichlet Allocation} (LDA) estão entre os algoritmos mais populares e foram desenvolvidos, respectivamente, com os propósitos de: i) identificar posicionamento ideológico implícito; ii) mensurar a associação entre palavras e expressões e escalas númericas e iii) extrair agrupamentos temáticos de documentos \cite{slapin2008scaling, laver2003extracting, blei2003latent}. 

Entre as vantagens apresentadas pelos avanços no desenvolvimento de algoritmos, a possibilidade de abordar vários componentes da linguagem como unidades de análise está entre um deles. Classificação e extração de \emph{features} de documentos, tópicos, entidades textuais, sentenças e agentes de discurso permite seu uso na solução de problemas de pesquisa diversos. Além disso, o préprocessamento de bases oriundas de qualquer fonte -- de Redes Sociais a biblitoecas de livros escaneados -- tem sido cada vez mais eficiente \cite{grimmer2013text, wilkerson2017large, gentzkow2019text}.   

Em edição dedicada às ``\emph{Recent Innovations in Text Analysis for Social Science}'', no periódico da \emph{Political Analysis}, os artigos reunidos constituem bons exemplos de  aplicações de análise automatizada de conteúdo para identificação de i) informação sobre etnicidade a partir de nome, ii) de posicionamento ideológico latente, iii) \emph{expressed agenda} e iv) classes de documentos \cite{roberts2016introduction}. Publicações na nacionais dedicadas a \emph{clusterização} de discursos e de proposições de parlamentares \cite{batista2016mensurando, moreira2016palavra} contribuem como evidências de crescente usabilidade das técnicas.

Análise automatizada de conteúdo também tem sido utilizada na identificação de atitudes e comportamento de agentes políticos. 
A partir de técnicas de codificiação baseadas em dicionário de termos, trabalhos foram capazes de replicar, com relativa precisão, a classificação de realizada por codificadores individuais de discursos populistas \cite{pauwels2011measuring, rooduijn2011measuring, oliver2016rise}. De forma similar, outras investigações utilizaram processamento de linguagem natural para automatização de partes do processo de identificação, como o reconhecimento de entidades associadas a adjetivos populistas \cite{kyle2018populists} e detecção de \emph{triplets} -- tríades compostas por sujeito, verbo e predicado \cite{aslanidis2018measuring}.

Para além da classificação de atributos atitudinais, alogirmtos de \emph{machine learning} também têm sido capazes de modelar fenômenos psicológicos complexos como discurso de ódio \cite{davidson2017automated, zampieri2019predicting}, hostilidade \cite{hopp2017does, vargo2017socioeconomic, liu2018forecasting}, agressividade \cite{orasan2018aggressive, sahay2018detecting, nikhil2018lstms} e depressão \cite{mowery2016identifying, chekroud2016cross, orabi2018deep}. Portanto, é intuitivo crer que as vantagens das quais uma grande literatura têm se beneficiado no uso de técnicas de análise automatizada de conteúdo também podem ser extendidas aos estudos sobre autoritarismo entre agentes políticos em sistemas democráticos.

Todavia, algumas ressalvas importantes quanto a natureza dos modelos estatísticos de texto precisam ser salientadas. \citeonline{grimmer2013text} apresentou os \emph{4 Princípios da Análise Quantiva de Texto}, são eles:
...
Os tópicos sintetizam as maiores dificuldades da literatura que consome as novas ténicas de processamento de texto. Há grande perda de informação do valor semântico das palavras e frases quando elas são transformadas em vetores de números. Além disso, modificações no corpus textual analisado pode impactar em significativas mudanças nas estimativas realizadas. Por fim, a validação e, principalmente, a consistência dos resultados apresentados nos trabalhos ainda permanece numa zona relativamente obscura. 

\subsection{Modelo de Classificação de Agentes Políticos Autoritários}
